...
def gaze_to_visual_angle(
    ...
    """
    Convert gaze position in pixels to visual angle in degrees.
    Returns theta_x_deg, theta_y_deg, theta_radial_deg.
    """
    cm_per_px_x = screen_w_cm / screen_w_px
    cm_per_px_y = screen_h_cm / screen_h_px

    dx_cm = (x_px - x_center_px) * cm_per_px_x
    dy_cm = (y_px - y_center_px) * cm_per_px_y

    theta_x = math.atan(dx_cm / viewing_distance_cm)
    theta_y = math.atan(dy_cm / viewing_distance_cm)

    theta_x_deg = math.degrees(theta_x)
    theta_y_deg = math.degrees(theta_y)
    theta_radial_deg = math.sqrt(theta_x_deg ** 2 + theta_y_deg ** 2)

    return theta_x_deg, theta_y_deg, theta_radial_deg
...
def process_video(video_path, input_method):
    ...
    # Load circle coordinates from circle_coordinates.csv (output of get_circle_coords.py)
    circle_coords_path = os.path.join(os.getcwd(), "circle_coordinates.csv")
    circle_coords = {}
    if os.path.exists(circle_coords_path):
        try:
            with open(circle_coords_path, "r", newline="") as f:
                reader = csv.DictReader(f)
                for row in reader:
                    try:
                        fid = int(row.get("frame", row.get("frame_id", -1)))
                        x_raw = row.get("x")
                        y_raw = row.get("y")
                        x_val = float(x_raw) if x_raw not in (None, "", "-1") else None
                        y_val = float(y_raw) if y_raw not in (None, "", "-1") else None
                        circle_coords[fid] = (x_val, y_val)
                    except Exception:
                        continue
            print(f"Loaded circle coordinates for {len(circle_coords)} frames from {circle_coords_path}")
        except Exception as e:
            print(f"Warning: could not load circle_coordinates.csv ({e}); angles will be None.")
    else:
        print("Warning: circle_coordinates.csv not found; angles will be None.")
    ...
    if input_method == 1:
        ...
        print("Duration:", frame_count / fps)
    ...
    frame_id = 0
    results = []
    blink_pixel_threshold = 500  # heuristic: near-zero dark pixels likely means a blink/closed eye
    viewing_distance_cm = 60.0  # adjust as needed
    ...
    while True:
        ...
        # Derive pupil metrics for this frame
        validity_id = 4  # default to data lost
        pupil_center = (None, None)
        pupil_diameter = None
        theta_x_deg = None
        theta_y_deg = None
        theta_radial_deg = None

        if pupil_rotated_rect and len(pupil_rotated_rect) >= 2:
            (cx, cy), (axis_a, axis_b) = pupil_rotated_rect[0], pupil_rotated_rect[1]
            if axis_a > 0 and axis_b > 0:
                pupil_center = (float(cx), float(cy))
                pupil_diameter = float((axis_a + axis_b) / 2.0)  # average of major/minor axes in pixels
                validity_id = 0  # good data

        if validity_id != 0:
            nonzero_counts = [
                cv2.countNonZero(thresholded_image_strict),
                cv2.countNonZero(thresholded_image_medium),
                cv2.countNonZero(thresholded_image_relaxed)
            ]
            if max(nonzero_counts) < blink_pixel_threshold:
                validity_id = 1  # likely blink/closed eye
            else:
                validity_id = 4  # data lost / ellipse not found

        # Use circle_coordinates.csv values for gaze/angles (not the pupil center)
        circle_x, circle_y = circle_coords.get(frame_id, (None, None))
        if circle_x is not None and circle_y is not None:
            theta_x_deg, theta_y_deg, theta_radial_deg = gaze_to_visual_angle(
                circle_x,
                circle_y,
                viewing_distance_cm,
                screen_w_px=frame.shape[1],
                screen_h_px=frame.shape[0],
                x_center_px=frame.shape[1] / 2,
                y_center_px=frame.shape[0] / 2,
            )

        print(f"frame={frame_id}, center={pupil_center}, diameter_px={pupil_diameter}, validity={validity_id}")
        results.append([
            frame_id,
            circle_x,
            circle_y,
    ...
    # Persist per-frame metrics
    output_csv = os.path.join(os.getcwd(), "pupil_metrics.csv")
    ...
    print(f"Saved pupil metrics to {output_csv}")
...
#     # second parameter: 1 = video file
#     process_video(video_path, 1)


if __name__ == "__main__":
    # img21 = cv2.imread("eye1.png")
    # test = process_frame(img21)
    # print(test)
    #select_video()
